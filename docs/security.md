...
1. **Database** (`audit_logs` table) — For querying, compliance, and the admin dashboard
...
| **Data** | `cv_created`, `cv_deleted` | INFO |
| | `cv_exported`, `data_exported` | INFO |
| **Admin** | `user_deactivated/reactivated` | **ALERT** |
...
Audit logs can be queried via the admin API:
