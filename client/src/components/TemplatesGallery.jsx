...
const TemplatesGallery = ({ onSelectTemplate, onBack }) => {
  ...
  const ResumePreview = ({ template }) => {
    ...
    const activeSections = sidebarOrder
      ? sidebarOrder.filter((s) => visibleSections && visibleSections[s])
      : [];
            ...
            <div className="preview-sidebar-col preview-sidebar-col-light">
              {activeSections.slice(0, 3).map((_, idx) => (
                <div key={idx} className="preview-section-inline">
                  <div
                    className="preview-heading-inline"
                    style={{ borderColor: settings.accentColor }}
                  ></div>
                  <div className="preview-line"></div>
                  <div className="preview-line short"></div>
      ...
      <>
        ...
        <div className="preview-closing">
          ...
          <div
            ...
            style={{ borderBottomColor: settings.accentColor || "#1a1a1a" }}
          ></div>
        </div>
      </>
    );
      ...
      return (
        <div className="template-preview-mini cover-letter-preview cl-preview-modern">
          ...
          <div className="cl-preview-inner">
            ...
            >
              <div
                className="preview-name-cl"
                style={{ background: settings.sidebarColor1 || "#006666" }}
              ></div>
    ...
    if (clStyle === "executive") {
      return (
        <div className="template-preview-mini cover-letter-preview cl-preview-executive">
          ...
          <div
            className="cl-preview-gold-rule"
            style={{ background: settings.accentColor || "#7c6f57" }}
          ></div>
          <div className="cover-letter-body">{bodyContent}</div>
        </div>
      );
      ...
      <div className="gallery-content">
        ...
        <div className="type-filters">
          {documentTypes.map((docType) => (
            ...
            >
              <i
                className={`fas ${docType.id === "resume" ? "fa-file-alt" : docType.id === "cover-letter" ? "fa-envelope" : "fa-th-large"}`}
              ></i>
          ...
          {filteredTemplates.map((template) => (
            ...
            >
              ...
              <div className="template-info">
                <h3>
                  {t(`templates.items.${template.id}.name`, template.name)}
                </h3>
                <div className={`template-type-badge ${template.type}`}>
                  <i
                    className={`fas ${template.type === "cover-letter" ? "fa-envelope" : "fa-file-alt"}`}
                  ></i>
                  {t(
                    `templates.types.${template.type}-singular`,
                    template.type,
                  )}
                </div>
                <p>
                  {t(
                    `templates.items.${template.id}.description`,
