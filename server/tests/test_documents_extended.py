...
class TestDocumentVersions:
    ...
    def test_create_version_on_other_users_doc_returns_404(
        ...
        admin_doc = Document(title="Admin Doc", document_type="resume", data={}, owner_id=admin_user.id)
        db.add(admin_doc)
        db.commit()
        db.refresh(admin_doc)

        response = client.post(
            f"/api/documents/{admin_doc.id}/versions",
            headers=auth_headers,
...
class TestDocumentOwnershipGuards:
    ...
    def test_export_other_users_document_returns_404(
        ...
        admin_doc = Document(title="Admin Export", document_type="resume", data={}, owner_id=admin_user.id)
        db.add(admin_doc)
        db.commit()
        db.refresh(admin_doc)

        response = client.get(f"/api/documents/{admin_doc.id}/export", headers=auth_headers)
        assert response.status_code == 404
    ...
    def test_revoke_share_link_other_user_returns_404(
        ...
        admin_doc = Document(
            ...
            share_token="admin-share-token",
        )
        db.add(admin_doc)
        db.commit()
        db.refresh(admin_doc)

        response = client.delete(f"/api/documents/{admin_doc.id}/share", headers=auth_headers)
        assert response.status_code == 404
    ...
    def test_remove_profile_image_other_user_returns_404(
        ...
        response = client.delete(f"/api/documents/{admin_doc.id}/profile-image", headers=auth_headers)
        assert response.status_code == 404
