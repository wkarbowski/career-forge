...
class TestDocumentRetrieval:
    ...
    def test_get_other_user_document(
        ...
        response = client.get(f"/api/documents/{admin_doc.id}", headers=auth_headers)
...
class TestDocumentDeletion:
    ...
    def test_delete_other_user_document(
        ...
        response = client.delete(f"/api/documents/{admin_doc.id}", headers=auth_headers)
        assert response.status_code == 404

        # Verify document still exists
        still_exists = db.query(Document).filter(Document.id == admin_doc.id).first()
